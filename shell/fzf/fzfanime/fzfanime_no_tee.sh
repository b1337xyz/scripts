...
declare -r -x PLAYER='mpv --profile=big-cache'
...
function play {
    ...
    notify-send -t 1000 -i mplayer "MPV" "Anime: $1" 2>/dev/null || printf 'Notification failed\n'
    $PLAYER "${ANIME_DIR}/$1" &
    echo "$1" >> "$ANIME_HST"
}
function main() {
    case "$1" in
        ...
        offline)
            grep -xFf <(stat -c '%N' "$ANIME_DIR"/* | grep -vP '(onedrive|gdrive)' |
                awk -F' -> ' '{print $1}' | cut -d'/' -f6 | sed 's/.$//g') "$mainfile" > "$tmpfile"
        ;;
        by_score)
            grep -xFf "$mainfile" <(
            jq -r '[ keys[] as $k | .[$k] | {"title": $k, "score": .["score"]}] | sort_by(.score) | .[].title' "$DB") > "$tmpfile"
        ;;
        by_year)
            sed 's/.*(\([0-9]\{4\}\)).*/\1;\0/g' "$mainfile" | sort -n | sed 's/^[0-9]\{4\}\;//g' > "$tmpfile"
        ;;
        by_episodes)
            grep -xFf "$mainfile" <(
            jq -r '[keys[] as $k | {id: "\($k)", episodes: .[$k]["episodes"]}] | sort_by(.episodes)[] | .id' "$DB") > "$tmpfile"
        ;;
        watched)
            grep -xFf "$mainfile" "$WATCHED_FILE" | tac > "$tmpfile"
        ;;
        unwatched)
            grep -xvFf "$WATCHED_FILE" "$mainfile" > "$tmpfile"
        ;;
        history)
            grep -xFf "$mainfile" <(tac "$ANIME_HST" | awk '!seen[$0]++') > "$tmpfile"
        ;;
        continue)
            grep -vxFf "$WATCHED_FILE" <(grep -xFf "$mainfile" <(
                tac "$ANIME_HST" | awk '!seen[$0]++')) > "$tmpfile"
        ;;
        genre) 
            printf "genres" > "$mode"
        ...
        select)
            ...
            if [ "$curr_mode" = genres ];then
                ...
                stat -c '%N' "$ANIME_DIR"/* |
                    awk -F' -> ' '/'"$2"'/{split($1, a, "/"); x=a[length(a)]; print substr(x, 1, length(x) - 1) }' > "$tmpfile"
            else
                play "$2"
                cat "$mainfile"
            fi
        ;;
        nsfw)
            jq -Sr 'keys[] as $k | select(.[$k].isAdult) | $k' "$DB" > "$mainfile"
        ;;
        *)
