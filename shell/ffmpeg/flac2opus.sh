...
do
    ...
    # ffmpeg -hide_banner -nostdin -i "$i" -map_metadata 0 -map 0 \
    #     -c copy -c:a libopus -b:a 128k "${i%.*}.opus" && rm -v "$i"
    ffmpeg -hide_banner -nostdin -i "$i" -map 0 -map_metadata 0 -f flac - | opusenc - "${i%.*}.opus" && rm -v "$i"
